from ctcmodel import model
import tensorflow as tf
from tqdm import tqdm
from tensorflow.keras import optimizers
from ctcdataloader import train_iterator
import math
from utils import *
...
def train(model, data_iterator, optimizer):
    l = 0
    for i in tqdm(range(int(4650/32))):
...
if __name__ == '__main__':
    ...
    model = model(input_shape=[64, 9, 1])
    model.build(input_shape=(None,) + (64,9,1))

    # model = tf.keras.models.load_model("./h5/pose-155.h5")
    model.load_weights('./h5/tof-7x3-288.h5')
    ...
    for epoch_num in range(400):
        ...
        if epoch_num%10==0:
            model.save('./h5/tof-7x3-%s.h5'%epoch_num, save_format='h5')
