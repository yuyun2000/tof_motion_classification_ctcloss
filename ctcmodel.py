...
def model(input_shape, dropout_rate=0.5):
    ...
    x = conv_block(inputs, 16, 1,kernel_size=(7,3), strides=(2, 2))
    x = conv_block(x, 32, 1,kernel_size=(7,3))
    # x = conv_block(x, 32, 1)
    x = depthwise_conv_block(x, 64, 1, 1, (1, 1))
    x = depthwise_conv_block(x, 32, 1, 1, (1, 1))
    x = conv_block_withoutrelu(x, 16,kernel_size=(7,3), strides=(2, 2))
    # x = depthwise_conv_block(x,16,1,1,(2,2))
    # x = depthwise_conv_block(x, 64, 1, 1, (1, 1))
    ...
    x = layers.Reshape(target_shape=(-1, 3*16))(x)
    x = layers.Dropout(rate=dropout_rate)(x)

    x = layers.Dense(8, use_bias=True)(x)
    x = layers.Dense(4,use_bias=True)(x)
...
if __name__ == '__main__':
    # 获得模型结构
    model = model(input_shape=[64, 9, 1])  # 随即杀死神经元的概率

    # 查看网络模型结构
    # model.summary()
    # model.save("./mbtest.h5", save_format="h5")
    # print(model.layers[-3])
    model.load_weights('./h5/tof-7x3-310.h5')
    model.save('./h5/tof-7x3-310.h5' , save_format='h5')